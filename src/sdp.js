...
/* ========================= CODEC REGISTRY =========================
 *
 * SINGLE SOURCE OF TRUTH for codecs, replacing three lists that used to
 * disagree: the SDP wire tables here, the mimeType map in api.js that
 * fed getCapabilities(), and media-processing's own encoder list. The
 * disagreement was observable — getCapabilities() advertised PCMA and
 * PCMU while every offer we generated contained only opus, so an app
 * could ask "do you support PCMA?", be told yes, prefer it, and still
 * get opus on the wire with no error anywhere.
 *
 * Each entry carries EVERYTHING the three consumers need:
 *   name        — the SDP rtpmap name (what goes on the wire)
 *   mimeType    — the WebRTC identity (what getCapabilities reports)
 *   mpName      — media-processing's encoder name, or null when the
 *                 codec needs no encoder (telephone-event is generated
 *                 by the DTMF sender, not by an audio encoder)
 *   pt          — the STATIC payload type from RFC 3551 where one
 *                 exists (PCMU=0, PCMA=8, G722=9); null means dynamic
 *   clockRate / channels / fmtp / feedback / rtx — wire parameters
 *
 * LAYERING (one direction, no cycles):
 *   sdp.js  declares the registry (wire facts only, zero imports)
 *     -> media_session_factory builds m-sections from it
 *     -> api.js derives getCapabilities() from it, intersected with
 *        what media-processing reports it can actually encode
 * Nothing may hard-code a codec list anywhere else.
 */
var CODEC_REGISTRY = {
  audio: [
    { name: 'opus', mimeType: 'audio/opus', mpName: 'opus', pt: null,
      clockRate: 48000, channels: 2,
      fmtp: { minptime: 10, useinbandfec: 1 }, feedback: ['transport-cc'] },
    // G.711 (RFC 3551 §4.5.14) — 8 kHz mono only, STATIC payload types.
    // media-processing encodes both laws; rtp-packet packetizes them.
    // These are the baseline codecs for SIP/PSTN interop.
    { name: 'PCMU', mimeType: 'audio/PCMU', mpName: 'g711-ulaw', pt: 0,
      clockRate: 8000, channels: 1, fmtp: null, feedback: [] },
    { name: 'PCMA', mimeType: 'audio/PCMA', mpName: 'g711-alaw', pt: 8,
      clockRate: 8000, channels: 1, fmtp: null, feedback: [] },
    // DTMF (RFC 4733). Generated by the DTMF sender rather than an audio
    // encoder, so it has no media-processing name — but it IS on the
    // wire, and it belongs in getCapabilities (it was missing before).
    { name: 'telephone-event', mimeType: 'audio/telephone-event', mpName: null, pt: null,
      clockRate: 48000, channels: 1, fmtp: { '0-15': true }, feedback: [] },
  ],
  video: [
    // Feedback order matches Chrome for visual parity in dumps;
    // goog-remb is kept because Chrome still advertises it (legacy BWE).
    { name: 'VP8', mimeType: 'video/VP8', mpName: 'vp8', pt: null, clockRate: 90000,
      feedback: ['goog-remb', 'transport-cc', 'ccm fir', 'nack', 'nack pli'], rtx: true },
    { name: 'VP9', mimeType: 'video/VP9', mpName: 'vp9', pt: null, clockRate: 90000,
      feedback: ['goog-remb', 'transport-cc', 'ccm fir', 'nack', 'nack pli'], rtx: true },
    { name: 'H264', mimeType: 'video/H264', mpName: 'h264', pt: null, clockRate: 90000,
      feedback: ['goog-remb', 'transport-cc', 'ccm fir', 'nack', 'nack pli'], rtx: true,
      fmtp: { 'profile-level-id': '42e01f', 'level-asymmetry-allowed': 1, 'packetization-mode': 1 } },
    // H.265/HEVC (RFC 7798) — Safari negotiates natively, Chrome behind
    // a flag; rtp-packet ships the packetizer pair.
    { name: 'H265', mimeType: 'video/H265', mpName: 'h265', pt: null, clockRate: 90000,
      feedback: ['goog-remb', 'transport-cc', 'ccm fir', 'nack', 'nack pli'], rtx: true },
    // AV1 — media-processing encodes it and rtp-packet has av1.js, so it
    // is offered rather than only advertised (it used to appear in
    // getCapabilities alone).
    { name: 'AV1', mimeType: 'video/AV1', mpName: 'av1', pt: null, clockRate: 90000,
      feedback: ['goog-remb', 'transport-cc', 'ccm fir', 'nack', 'nack pli'], rtx: true },
  ],
};

// The wire tables are DERIVED — never edited directly.
function _wireTable(kind) {
  return CODEC_REGISTRY[kind].map(function (c) {
    var e = { name: c.name, clockRate: c.clockRate, feedback: c.feedback || [] };
    if (c.channels != null) e.channels = c.channels;
    if (c.fmtp) e.fmtp = c.fmtp;
    if (c.rtx) e.rtx = true;
    if (c.pt != null) e.pt = c.pt;
    return e;
  });
}

var DEFAULT_AUDIO_CODECS = _wireTable('audio');
...
var DEFAULT_VIDEO_CODECS = _wireTable('video');
...
/**
 * All a=msid values, per m-section, straight from the SDP TEXT.
 *
 * A track that belongs to several MediaStreams carries one a=msid line
 * per stream (W3C addTrack(track, s1, s2)), but sdp-transform keeps
 * only the FIRST of them and drops the rest entirely — they never even
 * reach m.invalid, so no amount of post-processing on the parsed object
 * can recover them. Scanning the raw text is the only way, and doing it
 * here keeps every consumer working off one parsed shape.
 *
 * @returns {Array<Array<string>>} one array of msid values per section
 */
function _msidsPerSection(sdpString) {
  var out = [];
  if (typeof sdpString !== 'string') return out;
  var sections = sdpString.split(/\r?\n(?=m=)/);
  for (var i = 1; i < sections.length; i++) {   // [0] is the session block
    var found = [];
    var lines = sections[i].split(/\r?\n/);
    for (var l = 0; l < lines.length; l++) {
      if (lines[l].indexOf('a=msid:') === 0) found.push(lines[l].slice(7).trim());
    }
    out.push(found);
  }
  return out;
...
function parseRemoteSdp(sdpString) {
  ...
  for (var i = 0; i < raw.media.length; i++) {
    ...
    var section = {
      ...
      msid: mediaMsid,
      // Every stream this track belongs to (see _msidsPerSection). One
      // entry for the common single-stream case, several when the app
      // called addTrack(track, s1, s2).
      msids: (_rawMsids[i] && _rawMsids[i].length) ? _rawMsids[i].slice()
           : (mediaMsid ? [mediaMsid] : []),
      // MULTI-STREAM STATUS: a track in several MediaStreams needs one
      // a=msid line per stream. sdp-transform keeps only the FIRST and
      // drops the rest entirely (they do not even reach m.invalid), so
      // completing this needs a raw-text scan before parsing — recorded
...
function buildAnswerMedia(offerMedia, config) {
  ...
  if (config.localSsrc && (config.localSsrc.msid || (config.localSsrc.msids && config.localSsrc.msids.length))) {
    // MULTI-STREAM (RFC 8830 + W3C addTrack(track, s1, s2)): ONE a=msid
    // line per stream the track belongs to. We emitted a single line, so
    // a receiver could never reconstruct the grouping the sender asked
    // for — the extra streams were dropped at the source.
    var _msidVals = (config.localSsrc.msids && config.localSsrc.msids.length)
      ? config.localSsrc.msids
      : [config.localSsrc.msid];
    m.msid = [];
    for (var _mv = 0; _mv < _msidVals.length; _mv++) {
      if (!_msidVals[_mv]) continue;
      var msidParts = String(_msidVals[_mv]).split(/\s+/);
      m.msid.push({ id: msidParts[0], appdata: msidParts[1] || undefined });
    }
    if (!m.msid.length) delete m.msid;
  }
  ...
  if (config.localSsrc) {
    ...
    function _emitLayerSsrcsA(primary, rtx, fec) {
      ...
      // One a=ssrc:<id> msid line PER stream the sender belongs to
      // (ssrc.msids), falling back to the single-stream token.
      var _msidList = (ssrc.msids && ssrc.msids.length) ? ssrc.msids
                    : (ssrc.msid ? [ssrc.msid] : []);
      for (var _mi = 0; _mi < _msidList.length; _mi++) {
        m.ssrcs.push({ id: primary, attribute: 'msid', value: _msidList[_mi] });
      }
...
function createOffer(config) {
  ...
  for (var i = 0; i < media.length; i++) {
    ...
    if (spec.type === 'application') {
      ...
      if (spec.ssrc && (spec.ssrc.msid || (spec.ssrc.msids && spec.ssrc.msids.length))) {
        // one a=msid line per stream (see the offer builder above)
        var _mv2 = (spec.ssrc.msids && spec.ssrc.msids.length) ? spec.ssrc.msids : [spec.ssrc.msid];
        m.msid = [];
        for (var _k2 = 0; _k2 < _mv2.length; _k2++) {
          if (!_mv2[_k2]) continue;
          var msidParts = String(_mv2[_k2]).split(/\s+/);
          m.msid.push({ id: msidParts[0], appdata: msidParts[1] || undefined });
        }
        if (!m.msid.length) delete m.msid;
      }
